...
|---|---|---|---|---|---|---|
| R1 | Profile bootstrap recursion between getAllProfiles and saveProfile on empty storage | Critical | Medium | App initialization failure in first-run scenarios | Refactor bootstrap to avoid saveProfile call inside getAllProfiles; add dedicated initializeProfiles function | src/services/profileService.ts |
...
| R3 | Category deletion behavior mismatches warning text (items not moved to Other) | High | High | Data loss and trust erosion | Implement transfer-to-Other logic before deletion and add tests | src/components/features/DeleteConfirmationModal.tsx, src/services/categoryService.ts, src/i18n/locales/en.json |
